...
%% name file
openstreetmap_filename = './mobilityFiles/SUMOFiles/mapFiltered.osm';
...
[parsed_osm, osm_xml] = externalFunc.openstreetmap.parse_openstreetmap(openstreetmap_filename);

%% find connectivity
[connectivity_matrix, intersection_node_indices] = externalFunc.openstreetmap.extract_connectivity(parsed_osm);
intersection_nodes = externalFunc.openstreetmap.get_unique_node_xy(parsed_osm, intersection_node_indices);
...
[route, dist] = externalFunc.openstreetmap.route_planner(dg, start, target);
...
[buildings, foliage, highway, buildingsIndexNaN, foliageIndexNaN, highwayIndexNaN] = externalFunc.openstreetmap.plot_way(ax, parsed_osm);
%plot_way(ax, parsed_osm, map_img_filename) % if you also have a raster image

% externalFunc.openstreetmap.plot_route(ax, route, parsed_osm)
% only_nodes = 1:10:1000; % not all nodes, to reduce graphics memory & clutter
% externalFunc.openstreetmap.plot_nodes(ax, parsed_osm, only_nodes)
