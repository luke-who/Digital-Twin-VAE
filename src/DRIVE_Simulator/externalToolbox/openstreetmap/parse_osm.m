...
for i=1:Nnodes
    ...
    hasTrafficLight = 0;
    if isfield(node{i},'tag')
        for k = 1:length(node{i}.tag)
            if length(node{i}.tag)==1
                toTest = {node{i}.tag};
            else
                toTest = node{i}.tag;
            end
            if strcmp(toTest{k}.Attributes.v,'traffic_signals')
            	hasTrafficLight = 1;
            end
        end
    end
    tagTraffic(1,i) = hasTrafficLight;
end
parsed_node.id = id;
parsed_node.xy = xy;
parsed_node.trafficLights = tagTraffic;
